-- ================================================================
-- Smart Shift Pro — Schema para Dokku PostgreSQL
-- Autenticación propia: password_hash (scrypt) + tabla sessions
-- Sin dependencias de Supabase Auth
--
-- Ejecutar contra la DB de Dokku (idempotente):
--   psql $DATABASE_URL -f supabase/schema_dokku.sql
-- ================================================================
...
CREATE INDEX IF NOT EXISTS idx_shift_history_emp_date   ON public.shift_history(employee_id, date);
CREATE INDEX IF NOT EXISTS idx_shift_history_changed_at ON public.shift_history(changed_at DESC);

-- ── 3. APROBACIONES DE REPORTES ─────────────────────────────────
...
CREATE INDEX IF NOT EXISTS idx_report_approvals_date ON public.report_approvals(date);
...
INSERT INTO public.roles (nombre, descripcion, permisos) VALUES
  ('admin',      'Administrador con acceso completo',
   '{"scheduler":"full","employees":"full","areas":"full","absences":"full","reports":"full","settings":"full"}'),
  ('supervisor', 'Supervisor operacional',
   '{"scheduler":"edit","employees":"edit","areas":"view","absences":"edit","reports":"view","settings":"view"}'),
  ('lider',      'Líder de área',
   '{"scheduler":"edit","employees":"view","areas":"view","absences":"edit","reports":"view","settings":"none"}'),
  ('gestor',     'Gestor de turnos',
   '{"scheduler":"edit","employees":"view","areas":"view","absences":"view","reports":"view","settings":"none"}'),
  ('consulta',   'Acceso de sólo lectura',
   '{"scheduler":"view","employees":"view","areas":"view","absences":"view","reports":"view","settings":"none"}')
...
CREATE INDEX IF NOT EXISTS idx_jornada_registros_emp_fecha ON public.jornada_registros(employee_id, fecha);
CREATE INDEX IF NOT EXISTS idx_jornada_registros_fecha     ON public.jornada_registros(fecha);
...
-- ── 9. TRIGGER updated_at para user_profiles ────────────────────
